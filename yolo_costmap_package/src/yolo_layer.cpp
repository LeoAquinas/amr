...
void YoloLayer::onInitialize()
{
  ...
  if (!node) {
    RCLCPP_ERROR(rclcpp::get_logger("YoloLayer"), "Failed to lock node pointer during initialization");
    return;
  }
...
void YoloLayer::computeCaches() {
  ...
  for (int j = 0; j < cache_length_; ++j) {
    for (int i = 0; i < cache_length_; ++i) {
      ...
      if (distance > avoidance_radius_) continue;
...
void YoloLayer::pointcloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
  ...
  if ((now - last_callback_time_).seconds() < 0.1) {  // Limit to 10Hz
    return;
  }
  ...
  try {
    transform = tf_buffer_->lookupTransform(global_frame_, msg->header.frame_id, msg->header.stamp, tf2::durationFromSec(5.0));
...
void YoloLayer::onFootprintChanged()
{
  need_recalculation_ = true;
  RCLCPP_DEBUG(rclcpp::get_logger("nav2_costmap_2d"), 
    "YoloLayer::onFootprintChanged(): num footprint points: %lu",
...
unsigned char YoloLayer::computeCost(double distance) const {
  ...
  return static_cast<unsigned char>(
    (1.0 - factor) * (LETHAL_OBSTACLE - FREE_SPACE) + FREE_SPACE);
}
